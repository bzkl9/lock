...
local function shouldShowBillboard(player)
...
local function handleTrackStart(controller, track)
    ...
    local ok = pcall(function()
        animInstance = track.Animation
    end)

    if not ok or not animInstance then
        pcall(function()
            animInstance = { AnimationId = track.AnimationId }
        end)
    end
    ...
    if RUN_ANIM_IDS[animIdStr] then
        ...
        conn = track.Stopped:Connect(function()
            ...
            if controller.trackStopConns and controller.trackStopConns[track] then
                pcall(function()
                    controller.trackStopConns[track]:Disconnect()
                end)
...
local function scanCurrentTracks(controller)
    ...
    local ok, tracks = pcall(function()
        return controller.humanoid:GetPlayingAnimationTracks()
    end)
    if not ok or not tracks then return end

    for _, track in ipairs(tracks) do
        pcall(function()
            handleTrackStart(controller, track)
        end)
    end
end
...
local function attachAnimationWatcher(controller)
    ...
    if controller.animPlayedConn then
        pcall(function()
            controller.animPlayedConn:Disconnect()
        end)
...
local function setupPlayer(player)
    ...
    local oldStaminaValue = player:FindFirstChild("StaminaValue")
    if oldStaminaValue then
        oldStaminaValue:Destroy()
    end
    ...
    local function onCharacterAdded(character)
        if controller.humanoidWaitConn then
            pcall(function()
                controller.humanoidWaitConn:Disconnect()
            end)
        ...
        if controller.diedConn then
            pcall(function()
                controller.diedConn:Disconnect()
            end)
        ...
        local existingBillboard = character:FindFirstChild("StaminaBillboard")
        if existingBillboard then
            existingBillboard:Destroy()
        end

        if shouldShowBillboard(player) then
    ...
    controller.heartbeatConn = RunService.Heartbeat:Connect(function(dt)
        ...
        if controller.character and (not controller.hrp or not controller.hrp.Parent) then
            controller.hrp = controller.character:FindFirstChild("HumanoidRootPart")
                or controller.character:FindFirstChild("Torso")
                or controller.character:FindFirstChild("UpperTorso")
        end
...
playerRemovingConn = Players.PlayerRemoving:Connect(function(player)
    ...
    if c then
        ...
        if c.trackStopConns then
            for t, conn in pairs(c.trackStopConns) do
                if conn then
                    pcall(function() conn:Disconnect() end)
                end
            end
...
print("[StaminaTracker] Local script loaded - own stamina bar hidden, others visible.")

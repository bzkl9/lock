...
local function predictedAimPoint(myPos, targetChar, chargeKey, forceSnap, allowAdapt)
    ...
    -- NEW: invert adaptation if the target is already moving in the same direction
    -- as their preferred dodge.  When adaptation is active (enoughDodges)
    -- and the player begins the windup moving toward their expected dodge side,
    -- flip the lateral offset to the opposite side to trick them.
    if adaptAllowed and profAll and enoughDodges then
        local signApplied = (lateralApplied > 0) and 1 or ((lateralApplied < 0) and -1 or 0)
        if signApplied ~= 0 then
            local dirSign = nil
            if dominantPreDir then
                dirSign = dominantPreDir
            else
                local dotLR = lateralVel:Dot(rightDir)
                if dotLR > 0.05 then dirSign = 1 elseif dotLR < -0.05 then dirSign = -1 else dirSign = 0 end
            end
            if dirSign ~= 0 and dirSign == signApplied then

...
-- Additional margin added to the dynamic post‑windup recording duration.
-- The actual recording duration is computed as (distance/projectile_speed) + RECORD_EXTRA_MARGIN.
-- This accounts for the time it takes a projectile to reach the target plus a small buffer to
-- ensure late dodges are captured.
local RECORD_EXTRA_MARGIN = 0.2
...
local function startPostWindupRecording(targetChar, key)
    ...
    local samples = {}
    local startT = tick()
    -- dynamic record duration: compute projectile travel time plus margin
    local recordDuration
    do
        recordDuration = POST_RECORD_DURATION
        local shooter = LocalPlayer and LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart") or nil
        if shooter and tHRP then
            local dist = (shooter.Position - tHRP.Position).Magnitude
            local spd = baseProjectileSpeed
            if spd and spd > 0 then
                recordDuration = (dist / spd) + RECORD_EXTRA_MARGIN
            end
        end
    end
    local endT = startT + recordDuration
    ...
    conn = RunService.Heartbeat:Connect(function()
        ...
        if hrp then
            ...
            while #samples > 0 and (now - samples[#samples].t) > (recordDuration + 0.05) do
        ...
        if now >= endT or (targetChar:FindFirstChild("Humanoid") and targetChar.Humanoid.Health <= 0) then
            ...
            local durationScale = math.clamp((recordDuration / 1.5), 0.5, 1.0)
...
-- adaptation decision (unchanged except gating removal)
...
local function predictedAimPoint(myPos, targetChar, chargeKey, forceSnap, allowAdapt)
    ...
    -- NEW: invert adaptation if the target has been moving in the same
    -- direction as their preferred dodge prior to release.  To avoid
    -- oscillating between left and right each frame, we only use the
    -- dominant pre‑windup direction (if available) for this inversion.
    -- When adaptation is active (enoughDodges) and there exists a
    -- dominant pre‑windup run direction, we flip the lateral offset once
    -- if that direction matches the sign of the currently applied offset.
    if adaptAllowed and profAll and enoughDodges and dominantPreDir then
        local signApplied = (lateralApplied > 0) and 1 or ((lateralApplied < 0) and -1 or 0)
        if signApplied ~= 0 and signApplied == dominantPreDir then
...
_G.AimAssistConn = RunService.RenderStepped:Connect(function(dt)
    -- Automatically unlock/unhighlight dead targets.  If the currently
    -- locked target dies (health reaches zero or character disappears),
    -- disable aim assist and clear all locked visuals.  Similarly, if
    -- the highlighted target dies, remove the highlight.  Without this
    -- check the camera may remain glued to a dead target until the
    -- player manually unlocks.
    do
        -- Handle locked target death/unparenting
        if lockedTarget then
            local alive = true
            local hum = nil
            pcall(function() hum = lockedTarget:FindFirstChild("Humanoid") end)
            if not lockedTarget.Parent or not hum or hum.Health <= 0 then
                alive = false
            end
            if not alive then
                -- clear locked state and visuals
                aimActive = false
                -- if the locked target also has a red highlight, clear it below
                local deadTarget = lockedTarget
                lockedTarget = nil
                clearLockedTargetVisuals()
                -- If the dead target was also highlighted, clear the highlight
                if redHighlightedTarget == deadTarget then
                    clearRedHighlight()
                end
            end
        end
        -- Handle highlighted target death/unparenting (when not locked)
        if redHighlightedTarget and redHighlightedTarget ~= lockedTarget then
            local alive = true
            local hum = nil
            pcall(function() hum = redHighlightedTarget:FindFirstChild("Humanoid") end)
            if not redHighlightedTarget.Parent or not hum or hum.Health <= 0 then
                alive = false
            end
            if not alive then
                clearRedHighlight()
